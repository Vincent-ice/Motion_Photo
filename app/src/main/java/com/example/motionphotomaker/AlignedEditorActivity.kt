...
/**
 * Editor whose preview geometry is deliberately identical to CropMath:
 *
 * 1) the TextureView is laid out with the source display aspect ratio;
 * 2) the target-aspect FrameLayout clips that full source rectangle;
 * 3) EditorGestureView applies zoom/pan exactly once using normal View properties;
 * 4) export receives the same zoom/pan values and CropMath computes the same crop.
 *
 * We never call TextureView.setTransform() here. That avoids both ExoPlayer
 * transform conflicts and the previous double-application of zoom/pan.
 */
class AlignedEditorActivity : ComponentActivity() {
    ...
    private val ticker = object : Runnable {
        override fun run() {
            player?.let { p ->
                if (p.isPlaying && ::timeline.isInitialized) {
    ...
    private fun ensurePlayer(): ExoPlayer? {
        ...
        return try {
            ...
            p.addListener(
                object : Player.Listener {
                    ...
                    override fun onPlaybackStateChanged(playbackState: Int) {
                        playButton.text = if (p.isPlaying) "暂停" else "播放选区"
                    }

                    override fun onVideoSizeChanged(videoSize: VideoSize) {
                        // ExoPlayer may update its SurfaceTexture after prepare/seek.
                        // Our View-level geometry must remain the final authority.
            ...
            statusText.text = "播放器初始化失败：${t.javaClass.simpleName}: ${t.message}"
    ...
    private fun buildUi(): View {
        ...
        videoCard.addView(
            previewOuter,
            LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(370)),
        )
        ...
        previewOuter.addView(
            previewViewport,
            FrameLayout.LayoutParams(dp(208), dp(370), Gravity.CENTER),
        )

        // No background and no setTransform() on TextureView.
        ...
        gestureView = EditorGestureView(this).apply {
            onTransformChanged = { z, x, y ->
                ...
                // Do NOT call TextureView.setTransform here. EditorGestureView
                // already applied this exact state once through View properties.
            }
        }
        previewViewport.addView(
            gestureView,
        ...
        videoCard.addView(label("单指拖动画面，双指缩放；预览参数只应用一次。", 12f))
        ...
        trimInfo = label("拖动左右手柄设置切入 / 切出", 12f)
        ...
        root.addView(sectionTitle("视频缩放 / 取景"))
        ...
        coverOuter.addView(
            coverViewport,
            FrameLayout.LayoutParams(dp(174), dp(310), Gravity.CENTER),
        )
        coverCropView = CoverCropView(this).apply {
            targetAspect = this@AlignedEditorActivity.targetAspect
            onTransformChanged = { z, x, y ->
                coverZoom = z
                coverPanX = x
                coverPanY = y
    ...
    private fun loadVideo(uri: Uri) {
        ...
        try {
            ...
            trimInfo.text =
                "${formatTime(0L)} → ${formatTime(sourceDurationMs)} · ${"%.2f".format(sourceDurationMs / 1000.0)} s"
            videoInfo.text =
    ...
    /**
     * Lay the TextureView out as the full source rectangle using its display
     * aspect ratio. The target-aspect parent clips this rectangle. This is the
     * same geometric model used by CropMath before zoom/pan are applied.
     */
    private fun layoutVideoTextureBase() {
        ...
        val lp = FrameLayout.LayoutParams(textureWidth, textureHeight, Gravity.CENTER)
        videoTexture.layoutParams = lp
        videoTexture.requestLayout()

        // Preserve the user state after any player/layout update.
        videoTexture.post {
            gestureView.refreshPreviewTransform()
        }
    }

    private fun updateViewportSizes() {
        previewOuter.post {
            val maxW = (previewOuter.width - dp(12)).coerceAtLeast(dp(100))
            val (w, h) = fitAspect(targetAspect, maxW, dp(358))
            previewViewport.layoutParams = FrameLayout.LayoutParams(w, h, Gravity.CENTER)
            previewViewport.requestLayout()
            previewViewport.post { layoutVideoTextureBase() }
        }
        coverOuter.post {
            val maxW = (coverOuter.width - dp(12)).coerceAtLeast(dp(100))
            val (w, h) = fitAspect(targetAspect, maxW, dp(298))
    ...
    private fun updateVideoTransformInfo() {
        if (::transformInfo.isInitialized) {
            transformInfo.text =
                "视频：${"%.2f".format(videoZoom)}× · X ${"%+.0f".format(videoPanX * 100)}% · Y ${"%+.0f".format(videoPanY * 100)}%"
        }
    }
    ...
    private fun updateCoverTransformInfo() {
        if (::coverTransformInfo.isInitialized) {
            coverTransformInfo.text =
                "封面：${"%.2f".format(coverZoom)}× · X ${"%+.0f".format(coverPanX * 100)}% · Y ${"%+.0f".format(coverPanY * 100)}%"
        }
    }
    ...
    private fun generate() {
        ...
        statusText.text =
            "正在导出… 视频参数 ${"%.2f".format(videoZoom)}× / X ${"%+.0f".format(videoPanX * 100)}% / Y ${"%+.0f".format(videoPanY * 100)}%"
            ...
            try {
                ...
                runOnUiThread {
                    ...
                    statusText.text = buildString {
                        ...
                        append(
                            "已导出参数：${"%.2f".format(videoZoom)}× / X ${"%+.0f".format(videoPanX * 100)}% / Y ${"%+.0f".format(videoPanY * 100)}%",
                        )
                    }
                }
            } catch (t: Throwable) {
    ...
    private fun readVideoMeta(uri: Uri): VideoMeta {
        ...
        return try {
            ...
            if (rotation == 90 || rotation == 270) {
                VideoMeta(duration, height, width)
            } else {
    ...
    private fun openResult() {
        ...
        runCatching {
            ...
            statusText.text = "文件已经保存，但没有找到可打开它的相册应用。"
        }
    }
    ...
    private fun formatTime(ms: Long): String {
        ...
        val remain = seconds - minutes * 60
        return if (minutes > 0) {
            "%d:%05.2f".format(minutes, remain)
        } else {
            "%.2fs".format(remain)
        }
    }
